...
void MainGame::handleInput()
{
	...
	if (inputManager.isKeyPressed(SDLK_f)) {
		cout << "Humanos: " << totalHumans << '\n';
		cout << "Zombies: " << totalZombies << '\n';
	}
}
...
void MainGame::initLevel() {
	...
	player = new Player();
	player->init(5,10.0f, levels[currentLevel]->getPlayerPosition(), &inputManager);
	spriteBatch.init();
...
void MainGame::updateElements() {
	...
	for (size_t i = 0; i < zombies.size(); i++) {
		...
		if (zombies[i]->collideWithAgent(player)) {
			...
			alphaReduceTotal += alphaReduce;
			Color otherColor = player->getColor();
			otherColor.a = alphaReduceTotal;
			player->setColor(otherColor);
			if (player->isDead()) {
				cout << "Para revivir presione la tecla R\n";
			}
			break;
		}
